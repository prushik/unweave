...
#define X11_AUTH_FAMILY_LOCALHOST	0x00fc
#define X11_AUTH_FAMILY_KERBEROS	0x00fd
#define X11_AUTH_FAMILY_NETNAME		0x00fe
#define X11_AUTH_FAMILY_LOCAL		0x0100
#define X11_AUTH_FAMILY_ANY			0xffff
...
#define X11_OP_REQ_POLY_SEGMENT		0x42
#define X11_OP_REQ_POLY_ARC			0x44
...
//For the record, cause there is no doc on this, this is the format of the XAUTHORITY file
struct x11_authority
{
	uint16_t	family;
	uint16_t	addr_len;
	uint8_t		*addr;
...
void x11_grab_key(struct x11_connection *conn, uint8_t key, uint16_t modifiers,
					uint32_t win, uint8_t owner, uint8_t sync_key, uint8_t sync_mouse);

void x11_ungrab_kb(struct x11_connection *conn, uint32_t time);

void x11_warp(struct x11_connection *conn, uint32_t src, uint32_t dest,
					uint16_t src_x, uint16_t src_y, uint16_t w, uint16_t h, uint16_t x, uint16_t y);

uint8_t x11_get_ext(struct x11_connection *conn, uint16_t len, uint8_t *name);

void x11_poly_segment(struct x11_connection *conn, uint32_t target, uint32_t gc,
					void *data, int n);

void x11_poly_arc(struct x11_connection *conn, uint32_t target, uint32_t gc,
					void *data, int n);
//------------------------------------------------------------------------------
