...
//data should be &{uint16_t x1,y1,x2,y2}
void x11_poly_segment(struct x11_connection *conn, uint32_t target, uint32_t gc,
					void *data, int n)
{
	uint32_t packet[3];
	uint16_t length = (n*2) + 3;

	packet[0]=X11_OP_REQ_POLY_SEGMENT | length<<16;
	packet[1]=target;
	packet[2]=gc;

	write(conn->sock,packet,12);
	write(conn->sock,data,n*8);
...
{
	uint32_t packet[3];
	uint16_t length = (n*3) + 3;

	packet[0]=X11_OP_REQ_POLY_ARC | length<<16;
	packet[1]=target;
	packet[2]=gc;

	write(conn->sock,packet,12);
	write(conn->sock,data,n*12);
