...
//**															**
//**	PHilip RUshik. 2016										**
//**	Distributed under the terms of the Beerware License		**
//**	Philip Rushik wrote this code. As long as you retain	**
//**	this notice, you can do whatever you want with this		**
//**	stuff. If we meet someday, and you think this stuff		**
//**	is worth it, you can buy me a beer in return.			**
//**							--Philip Rushik					**
//**															**
//**			HTTP Unweave Server								**
//**															**
//**	Serves files and dynamic content over the HTTP.			**
//**															**
//****************************************************************
...
int parse_http_request(unsigned char *packet, struct http_request *request)
{
//HTTP REQUEST FORMAT
//	<METHOD> <URI> <VERSION>
//
...
//	<VERSION>	= HTTP/x.xx
	...
	request->METHOD=packet;

	int i;
	for (i=0;packet[i]!=' ' && packet[i]!='\0';i++) ;
	packet[i]='\0';

	request->URI=&packet[i+1];

	for (i++;packet[i]!=' ' && packet[i]!='\0';i++) ;
	packet[i]='\0';

	return (int)(&packet[i]-request->URI);
}
...
//	signal(SIGCHLD, SIG_IGN);
	...
	while (1)
	{
		...
		if (!pid)
		{
			...
			unsigned char buffer[PATH_MAX] = {0};
			n = read(openfd,buffer,PATH_MAX);
